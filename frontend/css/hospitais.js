...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!currentUser) {
    // Apenas mostra mensagem, não redireciona nem recarrega
    userName.textContent = 'Visitante';
    // Opcional: mostrar aviso na tela
    // document.body.insertAdjacentHTML('afterbegin', '<div class="notification-bar" style="background:#ffcccc;color:#900;padding:10px;text-align:center;">Faça login para acessar as funcionalidades.</div>');
    return;
  }
  
  userName.textContent = currentUser.nome.split(' ')[0]; // Corrigido para 'nome' vindo do backend
